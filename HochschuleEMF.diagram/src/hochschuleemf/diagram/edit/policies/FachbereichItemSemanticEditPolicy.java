 ...
 * @generated
 */
public class FachbereichItemSemanticEditPolicy extends HochschuleemfBaseItemSemanticEditPolicy {

	/**
	* @generated
	*/
	public FachbereichItemSemanticEditPolicy() {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (HochschuleemfVisualIDRegistry.getVisualID(node)) {
			case FachbereichFachbereichStudiengangCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (HochschuleemfVisualIDRegistry.getVisualID(cnode)) {
